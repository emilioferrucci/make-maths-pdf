...
- Do not treat the number or length of exchanges as a direct measure of mathematical importance. A topic may dominate because the user repeatedly struggled with one point or followed a temporary rabbit hole; consolidate such clarification loops and give the material only the space needed for a clear, proportionate treatment.
- Do treat intentional signals about emphasis as evidence. A request to move quickly, treat a point lightly, or defer details indicates that the topic should be covered more compactly. A deliberate request for details, derivations, examples, or a deeper exploration indicates that the corresponding part should be developed more fully. Distinguish requests for additional substance from repeated attempts to resolve the same confusion.
- Balance each topic using the current invocation, the user's explicit and implicit depth preferences, its role in the overall mathematical program, its dependencies, and the explanation the intended reader needs. Do not omit an in-scope concept merely to correct disproportionate transcript emphasis: preserve every distinct concept unless explicitly excluded or removed under the correction policy, using concise treatment rather than omission when it deserves less emphasis.
